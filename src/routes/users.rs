...
async fn login_user(
    credentials: web::Json<UserLoginInput>,
    conn: web::Data<PrismaClient>,
) -> Result<HttpResponse, AppError> {
    let credentials = credentials.into_inner();
    let email = credentials.email;
    ...
    let user = models::users::find_user(&conn, UserKey::Email(&email)).await?;
    ...
    match token {
        ...
        Some(s) => {
            ...
            match data {
                ...
                Ok(t) => {
                    let user =
                        models::users::find_user(&conn, UserKey::ID(t.custom.user_id)).await?;
